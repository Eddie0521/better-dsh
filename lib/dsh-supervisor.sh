...
case "${1:-status}" in start) start_profile ;; stop) stop_profile ;; restart) stop_profile || exit $?; start_profile ;; status) status_profile ;; *) echo "usage: dsh [start|stop|restart|status] | dsh web | dsh --profile web" >&2; exit 2 ;; esac
